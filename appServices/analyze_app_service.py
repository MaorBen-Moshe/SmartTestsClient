...
class AnalyzeAppService:
    def __init__(self, build_url: str, group_name: str):
        ...
        self.handle_group_data = HandleGroupsDataStep(self.data_manager.get_filter_for_curr_group())
        self.html_parser = HtmlParser(build_url)
    ...
    def analyze(self) -> dict[str, GroupData]:
        ...
        self.data_manager.groups_data = self.handle_group_data.init_groups_data()

        # analyze flows to run
        self.handle_group_data.analyze_flows_per_group(self.data_manager.services_map, self.data_manager.groups_data)
